...
def main() -> None:
    ...
    property_checks().to_csv(OUT / "population_property_checks.csv", index=False)
    standardisation_counterexample().to_csv(OUT / "standardisation_counterexample.csv", index=False)
    cluster_clone_counterexample().to_csv(OUT / "cluster_clone_counterexample.csv", index=False)
    ...
    print(f"Wrote P15 stress tests to {OUT}")
