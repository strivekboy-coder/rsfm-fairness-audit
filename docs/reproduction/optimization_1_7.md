...
The command freezes source hashes, compares GeoBWER with weighted SD and robust dispersion metrics, writes the complete slice table, renders ordered slice plots, builds the TerraMind cross-task table, and creates the available interaction atlas. It also writes `four_task_distribution_coverage.csv/json` and a two-panel comparator that machine-checks weighted SD, worst-minus-mean, GeoBWER, and full-slice coverage for fMoW-Sentinel, Sen1Floods11, reBEN, and AlphaEarth.
