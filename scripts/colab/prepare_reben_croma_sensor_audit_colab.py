...
def _write_blocked_report(path: Path, blocking: list[str], args: argparse.Namespace) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "Run this if the blocked report or preparation JSON shows `appdirs`, `fastcore.dispatch`, `bigearthnet_common`, `bigearthnet_patch_interface`, or `configilm` import failures:",
            "",
            "```bash",
